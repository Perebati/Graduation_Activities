...
class Disciplina:
    ...
    def getDados(self):
        dado = self.getCodigo() + ' -- ' + self.getNome() + '\n'
        return dado
...
class ConsultaDisciplinas(tk.Toplevel):
    def __init__(self, controle):

        tk.Toplevel.__init__(self)
        self.geometry('250x100')
        self.title("Consulta de disciplinas")
        self.controle = controle
        ...
        self.labelCodigo = tk.Label(self.frameNro,text="Cód da disciplina:")
        self.labelCodigo.pack(side="left")
        ...
        self.buttonSubmit = tk.Button(self.frameButton ,text="Enter")      
        self.buttonSubmit.pack(side="left")
        self.buttonSubmit.bind("<Button>", controle.checkDisciplina)
...
class CtrlDisciplina():       
    ...
    def consultaDisciplina(self):
        self.limiteIns = ConsultaDisciplinas(self)

    def checkDisciplina(self, event):
        codDisc = self.limiteIns.inputCodigo.get()
        for disc in self.listaDisciplinas:
            if disc.getCodigo() == codDisc:
                messagebox.showinfo("Disciplina encontrada!", disc.getDados())
                return
        messagebox.showinfo("Disciplina não encontrado!","A disciplina não existe no cadastro!") 
        return
