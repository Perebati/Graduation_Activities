...
class Estudante:
    ...
    def getDados(self):
        dado = self.getNroMatric() + ' -- ' + self.getNome() + '\n'
        return dado
...
class ConsultaEstudante(tk.Toplevel):
    def __init__(self, controle):

        tk.Toplevel.__init__(self)
        self.geometry('250x100')
        self.title("Consulta de estudante")
        self.controle = controle
        ...
        self.buttonSubmit = tk.Button(self.frameButton ,text="Enter")      
        self.buttonSubmit.pack(side="left")
        self.buttonSubmit.bind("<Button>", controle.checkEstudante)
...
class CtrlEstudante():       
    ...
    def consultaEstudante(self):
        self.limiteIns = ConsultaEstudante(self)

    def checkEstudante(self, event):
        nroMatric = self.limiteIns.inputNro.get()
        for est in self.listaEstudantes:
            if est.getNroMatric() == nroMatric:
                messagebox.showinfo("Aluno encontrado!", est.getDados())
                return
        messagebox.showinfo("Aluno não encontrado!","O aluno não existe no cadastro!") 
        return
